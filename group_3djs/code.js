...
var tg_group_chat_id = url.searchParams.get("tg_group_chat_id");
if (!tg_group_chat_id) {
    tg_group_chat_id = '';
}
       ...
       if(!url.searchParams.has('q') && !url.searchParams.has('f')){
		...
		document.querySelector('.pagination_count').innerHTML = url.searchParams.get('q');
			...
			if(getCookie("auth_token")=="" || getCookie("auth_token")==false){
				apiUrl = `${settings.api}api/getstats/user_connections_graph?from=${url.searchParams.get('f')}&number=${url.searchParams.get('q')}` +'&tg_group_chat_id=' + tg_group_chat_id;
			}
...
*/
	if(isAuth && !window.location.href.includes('id') && !window.location.href.includes('gen')){
		async function setProfilesLinks(){
	const response = await fetch(`${settings.api}api/getstats/user_connections_graph?from=${url.searchParams.get('f')}&number=${url.searchParams.get('q')}` +'&tg_group_chat_id=' + tg_group_chat_id, {
		method: "GET",
		headers: {
			"Authorization": 'Token ' + getCookie("auth_token")
		}
...
function initializeDisplay() {
	...
	node.append("image")
		...
		if(d.nodeType == NODE_TYPES.HOME){
			return `${window.location.origin}` + '/?tg_group_chat_id=' + tg_group_chat_id;
...
async function onNodeClick(nodeType, uuid, txt){
	...
	tgIframe.style.marginBottom = '0px';
		...
		window.location.href = settings.url + '?tg_group_chat_id=' + tg_group_chat_id;
	}
