...
if (getCookie("auth_data")) {
	...
	window.location.href = `${settings.url}trust/?id=${getCookie("user_uuid")}`;
}
...
recur_select.onchange = async function(){
	window.location.href = settings.url + 'trust/?id=' + url.searchParams.get('id') + '&d=' + this.value;
...
async function getApiUrl(){
	...
	if((getCookie("auth_token")=="" || getCookie("auth_token")==false) && !window.location.href.includes("id")){
		...
		if(url.searchParams.has('sl')){
			apiUrl = `${settings.api}api/profile_trust?uuid=${url.searchParams.get('id')}`;
			//try{
				await d3view();
			/*}catch(err){
			...
			if(url.searchParams.has('d')){
			 apiUrl = `${settings.api}api/profile_trust?uuid=${url.searchParams.get('id')}&depth=${url.searchParams.get('d')}`;
			}else{
				apiUrl = `${settings.api}api/profile_trust?uuid=${url.searchParams.get('id')}`;
			}
			///try{
		...
		apiUrl = `${settings.api}api/profile_trust?uuid=${getCookie('user_uuid')}&depth=${url.searchParams.get('d')}`;
		console.log(apiUrl)
		//try{
			await d3view();
		/*}catch(err){
...
async function onNodeClick(nodeType, uuid, txt){
    ...
    tgIframe.style.marginBottom = '0px';
			...
			window.location.href = `${settings.url}trust/?id=` + uuid;		
	}
		...
		window.location.href = url.origin + '/trust';
	}
...
async function OnfriendClickFunc(uid, nodeType){
	...
	function UserLink(){
		let txt = `${settings.url}trust/?id=` + uid;
...
function user_changed_info(id, last_name, first_name, middle_name, usr_photo, dob, dod, gender_val, us_latitude, us_longtitude){
  ...
  navigator.geolocation.getCurrentPosition(
	...
	async function getUsparent() {
		const response = await fetch(`${settings.api}api/profile_trust?uuid=${getCookie('user_uuid')}&depth=100`, {
		method: "GET",
		headers: {
			"Authorization": 'Token ' + getCookie("auth_token")
		}
	...
	async function get_info_about_parents() {
		...
		if(response.mother != null){
			moth_text.innerHTML = `${response.mother.last_name} ${response.mother.first_name} ${response.mother.middle_name} <a class="user_changed_link" href="${window.location.origin}/trust/?id=${response.mother.uuid}&d=5"><i class="fa fa-link" aria-hidden="true"></i></a>`;
			moth_text2_response = `${response.mother.last_name} ${response.mother.first_name} ${response.mother.middle_name} <a class="user_changed_link" href="${window.location.origin}/trust/?id=${response.mother.uuid}&d=5"><i class="fa fa-link" aria-hidden="true"></i></a>`;
		...
		if(response.father != null){
			fath_text.innerHTML = `${response.father.last_name} ${response.father.first_name} ${response.father.middle_name} <a class="user_changed_link" href="${window.location.origin}/trust/?id=${response.father.uuid}&d=5"><i class="fa fa-link" aria-hidden="true"></i></a>`;
			fath_text2_response = `${response.father.last_name} ${response.father.first_name} ${response.father.middle_name} <a class="user_changed_link" href="${window.location.origin}/trust/?id=${response.father.uuid}&d=5"><i class="fa fa-link" aria-hidden="true"></i></a>`;
