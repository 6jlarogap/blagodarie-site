...
function nextPage(){
	...
	if(window.location.href.includes('?page=' || '&page=')){
		if(localStorage.getItem('cur_page') < 9){
			...
			let new_int = window.location.search.slice(1, -2);
			+new_int;
			new_int++;
			console.log(new_int);
			localStorage.setItem('cur_page', new_int);
			window.location.assign(window.location.origin + window.location.pathname + window.location.search.slice(1, -2) + (+localStorage.getItem('cur_page')));
		}
	}
}
function prevPage(){
