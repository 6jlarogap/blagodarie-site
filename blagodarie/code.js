       ...
       if(!url.searchParams.has('q') && !url.searchParams.has('f')){
		...
		document.querySelector('.pagination_count').innerHTML = url.searchParams.get('q');
            ...
            if (userIdFrom != null && userIdTo != null && localStorage.getItem('filter') === null) {
                ...
                apiUrl = `${settings.api}api/profile_graph?uuid=${userIdTo}&from=${url.searchParams.get('f')}&number=${url.searchParams.get('q')}&query=` + localStorage.getItem('filter');
                //console.log(apiUrl);
            }
		console.log(apiUrl);
...
d3.json(apiUrl)
	...
	nodes.forEach(function(d) {
		...
		case KEYS_ROOT_ID:
			d.fx = width<900 ? width / 2+150 : width / 2 + 400;
			d.fy = width<900 ? height/2-90 : height / 2 - 200;
			break;
