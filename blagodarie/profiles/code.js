...
async function myProfilesinfo() {
	...
	for(let i = 0; i<response.length; i++){
		...
		tr.innerHTML = `<td><img src='${response[i].photo=="" ? settings.url+"/images/default_avatar.png" : response[i].photo}'/></td><td>${response[i].last_name + ' ' + response[i].first_name + ' ' + response[i].middle_name}</td><td>Нет данных<div class="user_changed"><a href="${window.location.origin}/?id=${response[i].uuid}"><i class="fa fa-link" aria-hidden="true"></i></a></div></td>`;
