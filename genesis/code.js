...
*/
	if(width<900){
		...
		simulation = d3.forceSimulation(nodes);
		simulation.force("link", d3.forceLink(links).id(d => d.id).distance(150).strength(1));
//		simulation.force("charge", d3.forceManyBody().strength(-13280).distanceMax(10050));
		simulation.force("x", d3.forceX(width / 2).strength(0.01));
		simulation.force("y", d3.forceY(height / 2).strength(0.01));		
		simulation.force("collide", d3.forceCollide().strength(1).radius(55));
		//simulation.force("center", d3.forceCenter(width / 2, height / 2))
