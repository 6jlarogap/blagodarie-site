...
*/
	...
	//	simulation.force("collide", d3.forceCollide().strength(5).radius(20));//.iterations(1));//radius 80  strength(0.6)
		...
		simulation = d3.forceSimulation(nodes);
		simulation.force("link", d3.forceLink(links).id(d => d.id).distance(110).strength(1));
		simulation.force("charge", d3.forceManyBody().distanceMax(300).strength(-1880));
		simulation.force("x", d3.forceX(width / 2));//.strength(0.025));
		simulation.force("y", d3.forceY(height / 2));//.strength(0.025));		
		simulation.force("collide", d3.forceCollide().strength(1).radius(95))//.iterations(1))
/*
